 ...
 * 
 * <pre>
 * com.wigo.cmm.common.taglib - CommCodeTag.java
 * </pre>
 *
...
//		List<String> selectedList = Utilities.isEmpty(selectedValue) ? new ArrayList<String>()
//				: new ArrayList<String>(Arrays.asList(selectedValue.split(",")));
		List<String> selectedList = new ArrayList<String>();
		if (Utilities.isNotEmpty(selectedValue)) {
			String[] arr = selectedValue.split(",");
			for (int i = 0; i < arr.length; i++) {
				String val = Utilities.nullCheck(arr[i]).trim();
		...
		try {
			...
			if (Utilities.isNotEmpty(prefixValue) || Utilities.isNotEmpty(prefixLabel)) {
				...
				options.append(prefixValue);
				options.append("\">");
				options.append(prefixLabel);
...
//				if (code.getComnCd().equals(selectedValue)) {
				...
				options.append(code.getComnCd());
				options.append("\" ");
				options.append(data);
				options.append(">");
				options.append(code.getComnCdNm());
				options.append("</option>");
			}
			...
			if (hasSelectTag) {
				...
				if (Utilities.isNotEmpty(className)) {
					sel.append("class=\"");
					sel.append(className);
					sel.append("\" ");
				}
				if (Utilities.isNotEmpty(change)) {
					sel.append("data-change=\"");
					sel.append(change);
					sel.append("\" ");
				}
				if (Utilities.isNotEmpty(upCombo)) {
					sel.append("data-up-combo=\"");
					sel.append(upCombo);
					sel.append("\" ");
				}
				if (Utilities.isNotEmpty(prefixLabel)) {
					sel.append("data-label=\"");
					sel.append(prefixLabel);
					sel.append("\" ");
				}
				if (Utilities.isNotEmpty(prefixValue)) {
					sel.append("data-label-value=\"");
					sel.append(prefixValue);
					sel.append("\" ");
