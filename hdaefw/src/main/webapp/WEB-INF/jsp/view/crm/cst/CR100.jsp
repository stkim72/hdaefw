...
<div id="wrapIframe" class="closed"> <!-- 고객정보 닫힘 -->
	<div class="cont">
	    <div class="gTitle1 line">
	        <h3 class="mTitle1">구매고객 검색</h3>
	    </div>
	        
	    <div class="mSort2 type2">
	        <form role="form" id="frmSearch">
			    <!-- 고객유형코드 [001 : 잠재고객 , 002 : 가망고객 , 003 : 체험고객 , 004 : 구매고객 , 005 : 추천고객 , 006 : 충성고객]        */  -->
				<input type="hidden" id="custTypeCd" name="custTypeCd" title="고객유형코드" value="004"> 
					<ul class="w100per">
						<li class="w12per">
							<div class="tit w100">판매구분</div>
							<div class="txt">
								<code:commCode id="saleSeCd" name="saleSeCd" codeCd="SD100" prefixLabel="전체" prefixValue=""/>
							</div>
						</li>
						<li class="w11per">
							<div class="tit w100">계약건수</div>
							<div class="txt">
								<input type="text" id="cnt" name="cnt" class="it" title="계약건수" oninput="this.value=this.value.replace(/[^0-9.]/g,'').replace(/(\..*)\./g,'$1');">
							</div>
						</li>
						<li class="w26per">
		...
		<div class="gTitle1">
			<h3 class="mTitle1">구매고객 목록</h3>
			<div class="gRt">
			    <a href="#" class="mBtn1 gray" data-auth-type="u" style="display:none" data-click="getDownloadReason">엑셀다운로드</a>
				<%-- <code:btnGroup name="blackList" hideDelete="true" hideInsert="true" hideSave="true" dispName="구매고객 목록" gridId="grdList"/> --%>
			</div>
		</div>
		...
		<div  id="divGrid" style="height:${rightBoxHeight4}" 
			...
			data-tpl-url="<c:url value='/static/gridTemplate/cst/CR100.xml'/>">
		</div>
	</div>
</div>
...
function showDetail(json){
	var url = "<c:url value='${urlPrefix}/detail${urlSuffix}'/>?itgCustNo="+json.itgCustNo+"&custNm="+json.custNm+"&saleSeCd="+json.saleSeCd;	
...
function changeText(reason){ 
    ...
    param.pfmWorkCd = "003";
    grdList.exportExcel("구매고객 목록.xlsx",true,null,param)
}
