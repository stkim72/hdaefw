...
<div id="wrapIframe" class="closed"> <!-- 고객정보 닫힘 -->
	<div class="cont">
	    <div class="gTitle1 line">
	        <h3 class="mTitle1">충성고객 검색</h3>
	    </div>
	        
	    <div class="mSort2 type2">
	        <form role="form" id="frmSearch">
			    <!-- 고객유형코드 [001 : 잠재고객 , 002 : 가망고객 , 003 : 체험고객 , 004 : 구매고객 , 005 : 추천고객 , 006 : 충성고객]        */  -->
				<input type="hidden" id="custTypeCd" name="custTypeCd" title="고객유형코드" value="006"> 
					<ul class="w100per">
						<li class="w11per">
							<div class="tit w100">총 구매횟수</div>
							<div class="txt">
								<input type="text" id="bosCntrtCnt" name="bosCntrtCnt" class="it" title="총 구매횟수" oninput="this.value=this.value.replace(/[^0-9.]/g,'').replace(/(\..*)\./g,'$1');">
							</div>
						</li>
						<li class="w11per">
							<div class="tit w100">총 추천횟수</div>
							<div class="txt">
								<input type="text" id="rcmdCnt" name="rcmdCnt" class="it" title="총 추천횟수" oninput="this.value=this.value.replace(/[^0-9.]/g,'').replace(/(\..*)\./g,'$1');">
							</div>
						</li>
						<li class="w26per">
                            <div class="tit w100">매출합계금액</div>
                            <div class="txt">
                                <div class="mDate1">
                                    <input type="text" class="it right comma" maxlength="13" data-type="number2" id="fromSaleAggrAmt" name="fromSaleAggrAmt">
                                    <span class="bar">~</span>
                                    <input type="text" class="it right comma" maxlength="13" data-type="number2" id="toSaleAggrAmt" name="toSaleAggrAmt">
                                </div>
                            </div>
                        </li>
						<li class="w20per">	
		...
		<div class="gTitle1">
			<h3 class="mTitle1">충성고객 목록</h3>
			<div class="gRt">
			    <a href="#" class="mBtn1 gray"  data-auth-type="u" style="display:none" data-click="getDownloadReason">엑셀다운로드</a>
				<%-- <code:btnGroup name="blackList" hideDelete="true" hideInsert="true" hideSave="true" dispName="층성고객 목록" gridId="grdList"/> --%>
			</div>
		</div>
		...
		<div  id="divGrid" style="height:${rightBoxHeight4}" 
			...
			data-tpl-url="<c:url value='/static/gridTemplate/cst/CR120.xml'/>">
		</div>
	</div>
</div>
...
function onGridCellDblClick(gridView, itemIndex, column, json, value){
	showDetail(json, column); 
}

/* 상세화면 팝업 로드(수정) */
function showDetail(json, column){
	
	var url = "";
	
	if(column == "bosCntrtCnt")
	{
		url = "<c:url value='${urlPrefix}/bosCntrtdetail${urlSuffix}'/>?itgCustNo="+json.itgCustNo+"&custNm="+json.custNm;
	}
	else if(column == "rcmdCnt")
	{
		url = "<c:url value='${urlPrefix}/rcmdDetail${urlSuffix}'/>?itgCustNo="+json.itgCustNo+"&custNm="+json.custNm;
	}
	else
	{
		return;
...
function changeText(reason){ 
    ...
    param.pfmWorkCd = "003";
    grdList.exportExcel("충성고객 목록.xlsx",true,null,param)
}
//setToday(); 
