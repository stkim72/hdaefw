    ...
    <!-- sort -->
           ...
           <!-- board -->
               <form role="form" id="frmSearch" name="frmSearch" onsubmit="search()">
				...
				<input type="hidden" id="stmpBasNo" name="stmpBasNo" value=""/>
         		
         		
    <div class="mSort2 mini"> <!-- class="mini" => 상하좌우 여백 좁은 사이즈-->
        <ul>
            <li class="wAuto ml20">
                <div class="tit">스탬프마스터명</div>
                <div class="txt">
                    <input type="text" class="it w90 data-check" title="스탬프마스터명"  name="stmpBasNm" id="stmpBasNm" value="">
                </div>
            </li>
            <li class="wAuto ml35">
                <div class="tit">스탬프마스터코드</div>
                <div class="txt">
                    <input type="text" class="it w90 " title="스탬프마스터코드" name="mshipStmpBasNo" id="mshipStmpBasNo" value="">
                    <span class="button">
...
function pubStmp(){
	...
	if( $("#frmSearch #stmpCnt").val() > 0  ){
		alert("이미 스탬프가 존재하는 고객입니다. ");
		return false;
	}
	
	if( confirm("스탬프를 발급하시겠습니까?") ){
		Utilities.getAjax(url, sndData, true, function(data, jqXHR) {
			if (Utilities.processResult(data, jqXHR, "스탬프발급을 실패하였습니다." )) {
				...
				if( data.cnt > 0 ){
					...
					$("#nStmpInfo").text("신규 멤버스 스탬프가 발급되었습니다.");
					$("#nStmpNo").show();
					$('#frmSearch')[0].reset();
